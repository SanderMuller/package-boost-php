...
Release-worthy features change user-visible behavior, so `README.md` and the `.ai/` files used locally (synced via `vendor/bin/boost sync`) can drift silently. Every release must audit both.
...
The `.ai/skills/` and `.ai/guidelines/` directories are synced by `boost-core` (`vendor/bin/boost sync`) to per-agent dirs (`.claude/skills/`, `AGENTS.md`, etc.). Generated files are gitignored — only `.ai/` sources are committed. Downstream projects get their own `.ai/` synced by the same `boost-core` plugin via their `boost.php`.
...
If any `.ai/` file changed, sync to refresh the generated per-agent files locally:

```bash
vendor/bin/boost sync || true
```

Generated files are gitignored — only the `.ai/` sources are committed. The Composer `post-install-cmd` hook re-runs this sync on every `composer install` / `composer update` in dev mode, so downstream contributors don't need to remember.
